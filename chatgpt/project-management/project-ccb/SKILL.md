...
> Aleut Federal company context lives in the **aleut-federal-context** Skill (auto-invoked for AF-related questions). The `operations/change-request` skill handles federal contract changes that require CO action; this skill handles **project-internal** change control. The two interact — out-of-scope changes detected here route to Contracts / CO via `operations/change-request`.
