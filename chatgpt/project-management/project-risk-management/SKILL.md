...
> Aleut Federal company context lives in the **aleut-federal-context** Skill (auto-invoked for AF-related questions). The `program-management/program-risk-management` skill handles program-level risk (spans multiple projects / task orders); this skill handles risk within a single project / task order. The two registers communicate — significant project risks roll up to the program register.
