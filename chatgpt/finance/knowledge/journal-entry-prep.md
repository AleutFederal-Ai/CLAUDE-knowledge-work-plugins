...
> **What this skill does:** Prepare journal entries with proper debits, credits, and supporting documentation for month-end close. Use when booking accruals, prepaid amortization, fixed asset depreciation, payroll entries, revenue recognition, or any manual journal entry.

# Journal Entry Preparation — Aleut Federal

> Aleut Federal company context lives in `ALEUT-FEDERAL-CONTEXT.md` (uploaded as `00-ALEUT-FEDERAL-CONTEXT.md`). See also the user-invocable `journal-entry` skill in this plugin for federal-contractor JE rules (direct vs indirect under FAR 31.202/203, FAR 31.205 allowability, CAS consistency, FAR 31.205-6(p) executive comp cap, segregation of unallowables).
