...
> **What this skill does:** Create effective data visualizations with Python (matplotlib, seaborn, plotly). Use when building charts, choosing the right chart type for a dataset, creating publication-quality figures, or applying design principles like accessibility and color theory.

# Data Visualization Skill — Aleut Federal

> Aleut Federal company context lives in `ALEUT-FEDERAL-CONTEXT.md` (uploaded as `00-ALEUT-FEDERAL-CONTEXT.md`). See the user-invocable `create-viz` skill for federal-audience visualization rules (Section 508, CUI banners, no agency logos, methodology footers, accessible PDFs).
