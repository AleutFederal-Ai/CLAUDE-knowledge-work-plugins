...
> **What this skill does:** Write optimized SQL for your dialect with best practices. Use when translating a natural-language data need into SQL, building a multi-CTE query with joins and aggregations, optimizing a query against a large partitioned table, or getting dialect-specific syntax for Snowflake, BigQuery, Postgres, etc.

# /write-query — Write Optimized SQL (Aleut Federal)

> Aleut Federal company context lives in `ALEUT-FEDERAL-CONTEXT.md` (uploaded as `00-ALEUT-FEDERAL-CONTEXT.md`). See the user-invocable `sql-queries` skill for federal-data hygiene (no CUI in snippets, audit logging assumptions, PII minimization, cost-data join alignment).
