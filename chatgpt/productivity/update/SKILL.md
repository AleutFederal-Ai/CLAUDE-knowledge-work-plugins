...
> Aleut Federal company context lives in the **aleut-federal-context** Skill (auto-invoked for AF-related questions). See `task-management` and `memory-management` for AF task/memory hygiene rules (charge-code tagging, CDRL handling, CUI prohibition, source-selection segregation, mandatory-disclosure routing).
